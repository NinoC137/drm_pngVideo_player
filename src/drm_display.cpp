...
DrmDisplay::~DrmDisplay() {
    for (int i = 0; i < 2; i++){    
        if (map_addr_) {
            munmap(map_addr_, buffer_[i].pitch * height_);
        }
        if (buffer_[i].fb_id) {
            drmModeRmFB(drm_fd_, buffer_[i].fb_id);
        }
        if (buffer_[i].handle) {
            drmIoctl(drm_fd_, DRM_IOCTL_MODE_DESTROY_DUMB, &buffer_[i].handle);
        }
        if (drm_fd_ >= 0) {
            close(drm_fd_);
        }
...
bool DrmDisplay::create_drm_buffer() {
    for(int i = 0; i < 2; i++) { //创建双缓冲区
        ...
        buffer_[i].handle = create_req.handle;
        buffer_[i].pitch = create_req.pitch;
    
        struct drm_mode_map_dumb map_req = {};
        map_req.handle = buffer_[i].handle;
        ...
        buffer_[i].map = static_cast<uint8_t *>(map_addr_);
    
        if (drmModeAddFB(drm_fd_, width_, height_, 24, 32, buffer_[i].pitch, buffer_[i].handle, &buffer_[i].fb_id) < 0) {
            std::cerr << "Failed to add framebuffer" << std::endl;
            return false;
        }
    }

    drmModeSetCrtc(drm_fd_, crtc_id_, buffer_[front_idx_].fb_id, 0, 0, &connector_id_, 1, &mode_);
...
bool DrmDisplay::load_png(const std::string &filename) {
    ...
    DrmBuffer &buf = buffer_[back_idx_]; //使用第二个缓冲区进行预装载
...
void DrmDisplay::display() {
    ...
    drmModePageFlip(drm_fd_, crtc_id_, buffer_[front_idx_].fb_id, DRM_MODE_PAGE_FLIP_EVENT, this);
    std::swap(front_idx_, back_idx_);
}
